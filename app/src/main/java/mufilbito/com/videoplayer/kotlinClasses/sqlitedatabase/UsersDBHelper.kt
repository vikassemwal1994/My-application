...
class UsersDBHelper(context: Context): SQLiteOpenHelper(context, DATABASE_NAME, null, DATABASE_VERSION) {
    ...
    fun readAllUsers(): ArrayList<UserModel> {
        ...
        try {
            cursor1 = db.rawQuery("select * from " + DBContract.UserEntry.TABLE_NAME, null)
        } catch (e: SQLiteException) {
            db.execSQL(SQL_CREATE_ENTRIES)
            return ArrayList()
        }
        ...
        if (cursor1!!.moveToFirst()) {
            while (cursor1.isAfterLast == false) {
                userid = cursor1.getString(cursor1.getColumnIndex(DBContract.UserEntry.COLUMN_USER_ID))
                name = cursor1.getString(cursor1.getColumnIndex(DBContract.UserEntry.COLUMN_NAME))
                age = cursor1.getString(cursor1.getColumnIndex(DBContract.UserEntry.COLUMN_AGE))
